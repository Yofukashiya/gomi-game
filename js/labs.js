...
const LABS = (() => {
  ...
  function jumpLab(D) {
    ...
    const P = {
      good: { jump: 12.5, grav: 60, lag: 0, coy: 120, buf: 150, t: '😍 <b>気持ちいい / Good Game Feel</b>：崖の許し（コヨーテ）と先行入力が効いている。ミスしても「自分のせい」に感じる。' },
      floaty: { jump: 16, grav: 26, lag: 0, coy: 120, buf: 150, t: '🌙 <b>月面 / Floaty</b>：滞空が長すぎて、空中で操作を待たされる。緊張感が消える。' },
      heavy: { jump: 7.5, grav: 120, lag: 0, coy: 0, buf: 0, t: '🧱 <b>モッサリ / Sluggish</b>：跳べる高さが足りず、許容もゼロ。同じコードでも別ゲーム。' },
      laggy: { jump: 12.5, grav: 60, lag: 100, coy: 120, buf: 150, t: '😩 <b>遅延100ms / Input Lag</b>：数値は同じ。入力が0.1秒遅れるだけで「壊れてる」と感じる。' },
      raw: { jump: 12.5, grav: 60, lag: 0, coy: 0, buf: 0, t: '🤖 <b>素の実装 / Naive</b>：<code>if (接地 &amp;&amp; 押した) 跳ぶ</code> だけ。正しいのに、押したのに跳ばない感じが出る。' },
    };
    const cfg = { ...P.good };
